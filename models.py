...
from utils.utils import build_targets, to_cpu, non_max_suppression, regular_dic
...
def create_modules(module_defs, regular_mode):
    """
    ...
    for module_i, module_def in enumerate(module_defs):
        ...
        if module_def["type"] == "convolutional":
            ...
            # dropblock
            if regular_dic["DROPBLOCK"] == regular_mode or regular_dic["BOTH"] == regular_mode:
                drop_prob = 0.1
                block_size = 7
            ...
            yolo_layer = YOLOLayer(anchors, num_classes, img_size, regular_mode)
...
class YOLOLayer(nn.Module):
    ...
    def __init__(self, anchors, num_classes, img_dim=416, regular_mode=regular_dic["NONE"]):
        ...
        self.regular_mode = regular_mode
    ...
    def forward(self, x, targets=None, img_dim=None):
        ...
        if targets is None:
            ...
            iou_scores, class_mask, obj_mask, noobj_mask, tx, ty, tw, th, tcls, tconf = build_targets(
                ...
                regular_mode=self.regular_mode,     # regular_mode
            )

            # 增加以下语句，修改bug
            obj_mask = obj_mask.bool() # int8 to bool
            noobj_mask = noobj_mask.bool() # int8 to bool
...
class DropBlock2D(nn.Module):
    ...
    def forward(self, x):
        ...
        if not self.training or self.drop_prob == 0.:
            ...
            # out__ = np.zeros((1,out_.shape[2],out_.shape[3]), dtype=np.int32)
            # out__ = out_[0, 0:1,:,:]
            # out__ = np.transpose(out__, [1,2,0]) 
            # out__ = cv.resize(out__, (0,0),fx=4,fy=4)
            # cv.imshow("img111", out__)
            # cv.waitKey(0)
...
class Darknet(nn.Module):
    ...
    def __init__(self, config_path, img_size=416, regular_mode=regular_dic["NONE"]):
        ...
        self.hyperparams, self.module_list = create_modules(self.module_defs, regular_mode)
