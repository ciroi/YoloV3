...
def load_classes(path):
    ...
    """
    fp = open(path, "r",  encoding='UTF-8') # 执行价 encoding='UTF-8'
    names = fp.read().split("\n")[:-1]
    return names
...
def build_targets(pred_boxes, pred_cls, target, anchors, ignore_thres, regular_mode=regular_dic["NONE"]):
    """
    generate masks & t·
    ...
    if regular_dic["SMOOTHLABEL"] == regular_mode or regular_dic["BOTH"] == regular_mode:
        # SmoothedLabel
        epsilon = 0.1 #𝑏𝑦 𝑑𝑒𝑓𝑎𝑢𝑙𝑡 
        smooth_label = epsilon * (1 - 1 / nC)
        tcls[...] = smooth_label
        tcls[b, best_n, gj, gi, target_labels] = 1 - smooth_label
        # print(tcls[b[0],best_n[0],gj[0],gi[0],])
    else:
